...
import type { Logger, McpbManifestLatest } from "../src/types";
...
describe("getMcpConfigForManifest", () => {
  ...
  const baseManifest: McpbManifestLatest = {
  ...
  it("should return undefined when manifest has no mcp_config", async () => {
    const manifest = {
      ...baseManifest,
      server: undefined,
    } as unknown as McpbManifestLatest;
  ...
  it("should return undefined when required config is missing", async () => {
    const manifest: McpbManifestLatest = {
      ...baseManifest,
      user_config: {
        apiKey: {
          type: "string",
