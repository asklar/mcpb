...
public static class PackCommand
{
    ...
    public static Command Create()
    {
        ...
        var forceOpt = new Option<bool>(
            name: "--force",
            description: "Proceed even if discovered tools differ from manifest"
        );
        var updateOpt = new Option<bool>(
            name: "--update",
            description: "Update manifest tools list to match dynamically discovered tools"
        );
        var noDiscoverOpt = new Option<bool>(
            name: "--no-discover",
            description: "Skip dynamic tool discovery (for offline / testing)"
        );
        ...
        var cmd = new Command("pack", "Pack a directory into an MCPB extension")
        {
            dirArg,
            outputArg,
            forceOpt,
        ...
        cmd.SetHandler(
            ...
            ) =>
            {
                ...
                {
                    Console.Error.WriteLine("ERROR: manifest.json not found");
                    return;
                }
                if (!ValidateManifestBasic(manifestPath))
                {
                    Console.Error.WriteLine("ERROR: Cannot pack invalid manifest");
                    return;
                }
                ...
                var outPath =
                    output != null
                        ...
                        : Path.Combine(
                            Directory.GetCurrentDirectory(),
                            SanitizeFileName(manifest.Name) + ".mcpb"
                        );
                ...
                {
                    var manifestTools =
                        manifest.Tools?.Select(t => t.Name).ToList() ?? new List<string>();
                    ...
                    if (listMismatch)
                    {
                        ...
                        Console.WriteLine(
                            "  Manifest:   [" + string.Join(", ", manifestTools) + "]"
                        );
                        Console.WriteLine(
                            "  Discovered: [" + string.Join(", ", discoveredToolNames) + "]"
                        );
                    }
                ...
                {
                    var manifestPrompts =
                        manifest.Prompts?.Select(p => p.Name).ToList() ?? new List<string>();
                    ...
                    if (listMismatch)
                    {
                        ...
                        Console.WriteLine(
                            "  Manifest:   [" + string.Join(", ", manifestPrompts) + "]"
                        );
                        Console.WriteLine(
                            "  Discovered: [" + string.Join(", ", discoveredPromptNames) + "]"
                        );
                    }

                    var metadataDiffs = ManifestCommandHelpers.GetPromptMetadataDifferences(
                        manifest.Prompts,
                        discoveredPrompts
                    );
                    if (metadataDiffs.Count > 0)
                ...
                if (mismatchOccurred)
                {
                    ...
                    else if (!force)
                    {
                        Console.Error.WriteLine(
                            "ERROR: Discovered capabilities differ from manifest. Use --force to ignore or --update to rewrite manifest."
                        );
                        Environment.ExitCode = 1;
                        return;
                    }
                    ...
                    {
                        Console.WriteLine(
                            "Proceeding due to --force despite capability mismatches."
                        );
                    }
                }
                ...
                var fileEntries = files
                    .Select(t => new
                    {
                        t.fullPath,
                        t.relative,
                        Size = new FileInfo(t.fullPath).Length,
                    })
                    .ToList();
                fileEntries.Sort(
                    (a, b) => string.Compare(a.relative, b.relative, StringComparison.Ordinal)
                );
                ...
                foreach (var fe in fileEntries)
                {
                    ...
                    if (parts.Length > 3)
                    {
                        ...
                        if (!deepGroups.TryGetValue(key, out var val))
                        {
                            val = (new List<string>(), 0);
                        }
                        val.Files.Add(fe.relative);
                        val.Size += fe.Size;
                        deepGroups[key] = val;
                    }
                    else
                        shallow.Add((fe.relative, fe.Size));
                }
                foreach (var s in shallow)
                    Console.WriteLine($"{FormatSize(s.Size).PadLeft(8)} {s.Rel}");
                foreach (var kv in deepGroups)
                {
                    ...
                    if (list.Count == 1)
                        ...
                        Console.WriteLine(
                            $"{FormatSize(size).PadLeft(8)} {kv.Key}/ [and {list.Count} more files]"
                        );
                }
    ...
    private static bool ValidateManifestBasic(string manifestPath)
    {
        try
        {
            ...
            return JsonSerializer.Deserialize(json, McpbJsonContext.Default.McpbManifest) != null;
        }
        catch
        {
            return false;
    ...
    private static List<(string fullPath, string relative)> CollectFiles(
        string baseDir,
